...
mcmcglmm_mod<-list()
for(chain in 1:3){
mcmcglmm_mod[[chain]]<-MCMCglmm(response~ height + social, random=~species,family="categorical",
                                 ginverse=list(species=inv.phylo$Ainv),prior=prior,
                                 data=df,nitt=25e3,burnin=5e3,thin=1e3), verbose=T,pr=T)
}
#save model
saveRDS( mcmcglmm_mod  , "output/models/ mcmcglmm_mod.rds")
...
A2<-ape::vcv.phylo(tree, cor=T) # cor=T is important, means you get correlation matrix from tree as in inverseA()
brms_mod <- brm(bf( response~ height + social  + 1|gr(species, cov = A)) ) + bernoulli(),
                  data = df,
                  data2 = list(A=A2),
