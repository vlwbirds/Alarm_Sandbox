...
inputdata <- data.frame(
  species = rep(c("SpeciesA", "SpeciesB", "SpeciesC"), each = 50),
  alarm = rbinom(150, 1, 0.3),  # Binary response variable
  column1 = rnorm(150, mean = 5, sd = 2),  # Continuous predictor
  column2 = rnorm(150, mean = 10, sd = 3), # Continuous predictor
  column3 = sample(1:5, 150, replace = TRUE)  # Categorical predictor
)
...
df <- inputdata %>% filter(species == "SpeciesA") #Define the specific species to filter

# Define predictor variables and interactions as character vectors
predictors <- c("column1", "column2", "column3")  # Replace with actual predictor column names
interactions <- c("column1:column2", "column1:column2:column3")  # Replace with actual interaction terms

# Build the predictor and interaction components of the formula
# Only include predictors if there are values in the 'predictors' vector
predictor_terms <- if (length(predictors) > 0) {
  paste(predictors, collapse = " + ")
} else {
  NULL
}

# Only include interactions if there are values in the 'interactions' vector
...
# If both are NULL, return an error; otherwise, construct formula
formula_str <- paste("alarm ~", paste(c(predictor_terms, interaction_terms), collapse = " + "))
formula_obj <- as.formula(formula_str)

# Fit the binomial GLM if there are predictor terms or interactions
if (!is.null(predictor_terms) || !is.null(interaction_terms)) {
  model <- glm(formula_obj, data = df, family = binomial)
  # View summary of the model
  summary(model)
} else {
  stop("No predictors or interactions defined for the model.")
}
...
# Species to filter, use 4-letter code lowercase (e.g., "haru")
species <- "haru"

# Filter the dataframe by species
df <- inputdata %>% filter(TarSpp == species) # Extract the specific species to filter

# Define predictor variables and interactions as character vectors
predictors <- c("Pre_Bird_Height", "Flight_proximity", "Height_Difference", "Veg_Under_Avg")  # Specify predictor column names
interactions <- c("Pre_Bird_Height:Flight_proximity", "Flight_proximity:Height_Difference")  # Define interaction terms if needed
...
# Fit the binomial GLM if there are predictor terms or interactions
if (!is.null(predictor_terms) || !is.null(interaction_terms)) {
  model <- glm(formula_obj, data = df, family = binomial(link = "logit"))
  # View summary of the model
  summary(model)
} else {
  stop("No predictors or interactions defined for the model.")
...
### GLM Spp Multiple Over Threshold
...
```{r}
view(read_csv(here("output/GLM_summaries_by_species_combinations.csv")))
```
...
# Convert appropriate columns to factors and numerics
df <- HwkGLM %>%
  mutate(across(c("Habitat_Primary",
                  "Habitat_Under_Dom1", 
                  "Habitat_Under_Dom2", 
...
# Add a derived column for height difference
df <- df %>%
  mutate(Height_Difference = Flight_apex - Pre_Bird_Height)

# Define predictor variables and interactions
predictors <- c("Pre_Bird_Height",
                "Bird_Dense_Avg",
                "Flight_proximity", 
                "Height_Difference",
...
interactions <- c("Flight_proximity:Pre_Bird_Height", "Height_Difference:Flight_proximity")

# Minimum sample size for a valid model run
min_sample_size <- 10
...
for (k in 1:length(predictors)) {
  ...
  for (combination in predictor_combinations) {
    ...
    if (!valid_combination) {
      message(paste("Skipping combination", paste(combination, collapse = ", "), "- insufficient levels."))
      next
    }
    ...
    # Filter rows with no missing data for the current combination
    filtered_df <- df %>% filter(rowSums(is.na(select(., all_of(combination)))) == 0)
    
    # Ensure sufficient sample size
    sample_size <- nrow(filtered_df)
    if (sample_size < min_sample_size) {
      message(paste("Skipping combination", paste(combination, collapse = ", "), "- insufficient data (sample size:", sample_size, ")."))
      next
    }
    
    # Fit the GLM
    model <- glm(formula_obj, data = filtered_df, family = binomial)
    
    # Get a tidy summary of the model
    model_summary <- tidy(model) %>%
      mutate(predictors = paste(combination, collapse = ", "),
             AIC = AIC(model),
             BIC = BIC(model),
             sample_size = sample_size)  # Add predictors, AIC, BIC, and sample size
    
    # Append the summary to the list
    model_summaries[[paste(predictor_terms, collapse = "_")]] <- model_summary
  }
}
...
# Export the combined summaries to a CSV file
write_csv(all_summaries, here("output/GLM_summaries_all_combinations.csv"))

cat("Model summaries saved to 'output/GLM_summaries_all_combinations.csv'\n")

```

```{r}
view(read_csv(here("output/GLM_summaries_all_combinations.csv")))
```
...
# Remove rows with NA in any predictor column before iteration
...
# Iterate over each predictor combination
for (combination in predictor_combinations) {
  # Check if all predictors in the combination have sufficient levels
  valid_combination <- all(sapply(combination, function(var) length(unique(filtered_df[[var]])) > 1))
  ...
  # Create interaction terms only if there are at least 2 predictors in the combination
  ...
  model <- glm(as.formula(formula_str), data = filtered_df, family = binomial)
  
  # Store the summary
  model_summaries[[paste(combination, collapse = "_")]] <- tidy(model) %>%
    mutate(predictors = predictor_terms,
           interactions = "None",
           AIC = AIC(model),
           BIC = BIC(model),
           sample_size = nrow(filtered_df))
  ...
  if (!is.null(interaction_terms)) {
    ...
    model_with_interactions <- glm(as.formula(formula_str_with_interactions), data = filtered_df, family = binomial)
    
    # Store the summary
    model_summaries[[paste(combination, collapse = "_", "interactions")]] <- tidy(model_with_interactions) %>%
      mutate(predictors = predictor_terms,
...
# Export the combined summaries to a CSV file
write_csv(all_summaries, here("output/GLM_summaries_with_interactions.csv"))

cat("Model summaries saved to 'output/GLM_summaries_with_interactions.csv'\n")


```

```{r}
view(read_csv(here("output/GLM_summaries_with_interactions.csv")))
```

subtract the mean and divide by the standard deviation

after in df \_std for standardized values

fit a global model, as complex as justifiable with all reasonable variables

reduce variables with excessively high inflation factor (VIF) performance packagel
...
```{r}
# Data
intercept <- -1.248057013
slope <- 0.056232165
standard_error <- 0.031827893

# Predictor range
flight_proximity_range <- seq(0, 20, length.out = 100)

# Calculate estimates and confidence intervals
estimate <- intercept + slope * flight_proximity_range
lower_bound <- estimate - 1.96 * standard_error
upper_bound <- estimate + 1.96 * standard_error

# Plot
plot(flight_proximity_range, estimate, type = "l", col = "blue", lwd = 2, 
     xlab = "Flight Proximity", ylab = "Estimate", 
     main = "GLM Linear Plot: Flight Proximity")
polygon(c(flight_proximity_range, rev(flight_proximity_range)), 
        c(lower_bound, rev(upper_bound)), 
        col = rgb(0, 0, 1, 0.2), border = NA)
abline(h = 0, col = "black", lty = 2) # Zero line
legend("bottomright", legend = c("Estimate", "95% Confidence Interval"), 
       col = c("blue", rgb(0, 0, 1, 0.2)), lty = c(1, NA), 
       fill = c(NA, rgb(0, 0, 1, 0.2)), bty = "n")

```

```{r}
# Data
intercept <- -2.483993707
slope_under <- 1.282182799
slope_mid <- 0.356186483
interaction <- -0.239699977
se_under <- 0.485161982
se_mid <- 0.147238530
se_interaction <- 0.083460476

# Ranges for predictors
veg_under_height <- seq(0, 5, length.out = 100)
veg_mid_low <- seq(0, 5, length.out = 100)

# Create a grid for interaction
grid <- expand.grid(veg_under_height = veg_under_height, veg_mid_low = veg_mid_low)

# Calculate estimates
grid$estimate <- intercept +
  slope_under * grid$veg_under_height +
  slope_mid * grid$veg_mid_low +
  interaction * grid$veg_under_height * grid$veg_mid_low

# Plot: Main Effects
par(mfrow = c(1, 2)) # Split plotting area for two plots

# Plot 1: Veg_under_height
estimate_under <- intercept + slope_under * veg_under_height
lower_bound_under <- estimate_under - 1.96 * se_under
upper_bound_under <- estimate_under + 1.96 * se_under

plot(veg_under_height, estimate_under, type = "l", col = "blue", lwd = 2, 
     xlab = "Veg_under_height", ylab = "Estimate", 
     main = "Effect of Veg_under_height")
polygon(c(veg_under_height, rev(veg_under_height)), 
        c(lower_bound_under, rev(upper_bound_under)), 
        col = rgb(0, 0, 1, 0.2), border = NA)
abline(h = 0, col = "black", lty = 2)

# Plot 2: Veg_mid_low
estimate_mid <- intercept + slope_mid * veg_mid_low
lower_bound_mid <- estimate_mid - 1.96 * se_mid
upper_bound_mid <- estimate_mid + 1.96 * se_mid

plot(veg_mid_low, estimate_mid, type = "l", col = "darkgreen", lwd = 2, 
     xlab = "Veg_mid_low", ylab = "Estimate", 
     main = "Effect of Veg_mid_low")
polygon(c(veg_mid_low, rev(veg_mid_low)), 
        c(lower_bound_mid, rev(upper_bound_mid)), 
...
library(lattice)
wireframe(estimate ~ veg_under_height * veg_mid_low, data = grid,
          xlab = "Veg_under_height", ylab = "Veg_mid_low", zlab = "Estimate",
          main = "Interaction: Veg_under_height * Veg_mid_low",
          col.regions = heat.colors(100), drape = TRUE, screen = list(z = 30, x = -60))
...
```{r}
tree <- consensus(nex_data, p = 0.5)
plot(tree, cex = 0.8)
```

### Plot Alarm Phylogeny
...
pruned_tree <- keep.tip(tree, spp_alarm$BirdTree)
...
# Prune the tree to include only species present in the alarm data
pruned_tree <- keep.tip(tree, df$BirdTree)

# Reorder alarm data to match pruned_tree$tip.label
alarm <- df$Alarm_Presence[match(pruned_tree$tip.label, df$Alarm_Presence)]
...
```{r}
unique(rooted_tree$tip.label)
unique(rooted_tree$node.label)
str(rooted_tree)
rooted_tree$node.label <- NULL

setdiff(unique(df$BirdTree), rownames(inv.phylo$Ainv))

unmatched_species <- setdiff(levels(cleaned_data$BirdTree), rownames(inv.phylo$Ainv))
print(unmatched_species)

# Find which levels are not represented in the data
missing_levels <- setdiff(rownames(inv.phylo$Ainv), levels(cleaned_data$BirdTree))
print(missing_levels) # Should be empty if everything is matched

# Optionally drop unused levels in cleaned_data
cleaned_data$BirdTree <- droplevels(cleaned_data$BirdTree)

table(cleaned_data$BirdTree)

# Get species names from BirdTree
birdtree_species <- levels(BirdTree)

# Compare with the reference data frame
reference_species <- unique(df$BirdTree)  # Replace with your actual column name

# Check for mismatches
missing_from_tree <- setdiff(reference_species, birdtree_species)
missing_from_reference <- setdiff(birdtree_species, reference_species)

if (length(missing_from_tree) > 0) {
    print(paste("Missing from BirdTree:", paste(missing_from_tree, collapse = ", ")))
}

if (length(missing_from_reference) > 0) {
    print(paste("Missing from reference df:", paste(missing_from_reference, collapse = ", ")))
}


# Print levels and their corresponding entries in the inverse matrix
for (species in levels(cleaned_data$BirdTree)) {
  if (!species %in% rownames(inv.phylo$Ainv)) {
    print(paste("Species not found in Ainv:", species))
  }
}
```
...
cleaned_data <- df %>% drop_na(Alarm_Presence, Pre_Bird_Height, SocialGroup, BirdTree)
# cleaned_data <- droplevels(cleaned_data)

rooted_tree$node.label <- paste0("node", 1:length(rooted_tree$node.label), rooted_tree$node.label)
...
mcmcglmm_mod<-list()
for(chain in 1:3){
  mcmcglmm_mod[[chain]]<-MCMCglmm(Alarm_Presence ~ Pre_Bird_Height + SocialGroup, random=~BirdTree, data = cleaned_data, family="categorical",
   ginverse=list((BirdTree=inv.phylo$Ainv), prior=prior, 
                 data=df,nitt=25e3,burnin=5e3,thin=1e3), verbose=T ,pr=T)
}
#save model
saveRDS( mcmcglmm_mod  , here("output/models/mcmcglmm_mod.rds"))

#diagnose model, assess satisfactory convergence
par(mfrow=c(3,2), mar=c(2,2,1,2)) #this is for 3 x 2 parameters, adjust as necessary
plot(do.call(mcmc.list, lapply( mcmcglmm_mod  , function(m) m$Sol)), ask=F) # look for fuzzy caterpillars
gelman.diag(do.call(mcmc.list,lapply( mcmcglmm_mod  , function(m) m$Sol))) #Rhat values of each parameter, should all be below 1.1
gelman.diag(do.call(mcmc.list,lapply( mcmcglmm_mod  , function(m) m$Sol)))$psrf[,1] %>% range #all need to be below 1.1
summary( mcmcglmm_mod[[1]])#Nef should all be in the 1000s, sometimes can be ok if only a few hundred, but best if high
#if convergence is good, then interpret
mcmcglmm_mod[[1]] %>% summary #perform inference on a single model's chain.

#if convergence is good, then interpret from all chains together
all_chains<-mcmcglmm_mod2[[1]]#initialize object
#add other chains in there
all_chains$Sol<-as.mcmc(rbind(mcmcglmm_mod2[[1]]$Sol, mcmcglmm_mod2[[2]]$Sol, mcmcglmm_mod2[[3]]$Sol))
all_chains$VCV<-as.mcmc(rbind(mcmcglmm_mod2[[1]]$VCV, mcmcglmm_mod2[[2]]$VCV, mcmcglmm_mod2[[3]]$VCV))
#all_chains$Liab<-as.mcmc(rbind(mcmcglmm_mod2[[1]]$Liab, mcmcglmm_mod2[[2]]$Liab, mcmcglmm_mod2[[3]]$Liab))#this can be needed, ignore if is NULL
summary(all_chains)  
...
# Load nexus file (100 trees)
nex_data <- read.nexus(here("data/trees/MayrAllErickson-tree-pruner-205594f1-a408-4345-b027-64dfdee069f9/output.nex"))
tree <- consensus(nex_data, p = 0.5)  # Generate a consensus tree
...
A2<-ape::vcv.phylo(rooted_tree, cor=T) # cor=T is important, means you get correlation matrix from tree as in inverseA()
brms_mod <- brm(bf( response~ Pre_Bird_Height + SocialGroup  + 1|gr(BirdTree, cov = A)) ) + bernoulli(),
                  data = df,
                  data2 = list(A=A2),
                  ...
                  file="output/models/mod_brms")

plot(brms_mod) #fuzzy caterpillars
summary(brms_mod) #verify Rhat <1.1 and ESS is high
#verify convergence then interpret
```
