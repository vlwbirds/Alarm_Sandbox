...
dggsave(here("figs/Alarm_Sociality_Individuals.png"))
```
...
glm_social1 <- glm(Alarm_Presence ~ SocialGroup + abundance + richness, data = cleaned_df, family = binomial(link = "logit"))
summary(glm_social1)
```
...
df_subset <- cleaned_df[, !names(cleaned_df) %in% unwanted_columns]

# Replace "#N/A" with NA in the Social_Index column
df_subset$SocialGroup[df_subset$SocialGroup == "#N/A"] <- NA
str(df_subset)
view(df_subset)
```
