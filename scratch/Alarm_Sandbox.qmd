...
# Check if hwk_subset has valid data
if (nrow(hwk_subset) == 0) {
  stop("No data available in hwk_subset.")
}

# Calculate percentages and sample sizes by SocialGroup
plot_data <- hwk_subset %>%
  group_by(SocialGroup) %>%
  summarise(
    Percentage = mean(alarm, na.rm = TRUE) * 100,
    SampleSize = n(),
    .groups = 'drop'  # This avoids warning about grouping structure
  ) %>%
  mutate(Group = recode(SocialGroup,
                        solo = "Solo",
                        pair = "Pair",
                        ssf = "Single-species Flock",
                        msf = "Mixed-species Flock",
                        ant = "Ant Flock",
                        lek = "Competitive Flock")) 
